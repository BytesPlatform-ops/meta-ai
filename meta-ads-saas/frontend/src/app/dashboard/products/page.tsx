...
export default function ProductsPage() {
  ...
  const [genDestType, setGenDestType] = useState<string>("WEBSITE");
  ...
  const [genLeadFormId, setGenLeadFormId] = useState("");
  const [genLeadFormName, setGenLeadFormName] = useState("");
  const [savedLeadForms, setSavedLeadForms] = useState<{ meta_form_id: string; form_name: string; questions: { type: string; key: string; label?: string }[] }[]>([]);
  const [leadFormsLoaded, setLeadFormsLoaded] = useState(false);
  const [aiFormLoading, setAiFormLoading] = useState(false);
  const [aiFormDraft, setAiFormDraft] = useState<{ form_name: string; questions: { type: string; key: string; label?: string }[]; reasoning: string } | null>(null);
  ...
  const openGenModal = (productId: string) => {
    ...
    setConversionEvent("");
    setPixelEvents([]);
    setGenDestType("WEBSITE");
    setGenWaNumber("");
    setGenMsgApps(["MESSENGER"]);
    setGenCallPhone("");
    setGenLeadFormId("");
    setGenLeadFormName("");
    setAiFormDraft(null);
    setLeadFormsLoaded(false);
    setLeadFormError(null);
    setShowGenModal(true);

    // Pre-fill WhatsApp number from preferences
  ...
  // Load saved lead forms + pages when INSTANT_FORM is selected
  useEffect(() => {
    if (genDestType !== "INSTANT_FORM" || leadFormsLoaded) return;
    setLeadFormsLoaded(true);
    api.listLeadForms().then((res: any) => {
      const data = res?.data || [];
      setSavedLeadForms(data);
    }).catch(() => {});
    api.fetchSocialIdentities().then((res: any) => {
      const pages = res?.data?.pages || [];
      setLeadFormPages(pages);
      if (pages.length > 0) setLeadFormPageId(pages[0].page_id);
    }).catch(() => {});
  }, [genDestType, leadFormsLoaded]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const destType = genDestType !== "WEBSITE" ? genDestType : undefined;
      const waNum = genDestType === "WHATSAPP" ? genWaNumber.trim() || undefined
        : genDestType === "MESSAGING" && genMsgApps.includes("WHATSAPP") ? genWaNumber.trim() || undefined
        : undefined;
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button type="button" onClick={() => setGenDestType("WEBSITE")}
                    className={`px-3 py-2.5 rounded-xl text-sm font-medium border transition-all ${genDestType === "WEBSITE" ? "bg-blue-500/10 border-blue-500/30 text-blue-400" : "bg-white/[0.03] border-white/[0.08] text-gray-500 hover:text-gray-300"}`}>
                    Website
                  </button>
                  <button type="button" onClick={() => setGenDestType("INSTANT_FORM")}
                    className={`px-3 py-2.5 rounded-xl text-sm font-medium border transition-all flex items-center justify-center gap-1.5 ${genDestType === "INSTANT_FORM" ? "bg-cyan-500/10 border-cyan-500/30 text-cyan-400" : "bg-white/[0.03] border-white/[0.08] text-gray-500 hover:text-gray-300"}`}>
                    <ClipboardList className="w-3.5 h-3.5" /> Instant Form
                  </button>
                  <button type="button" onClick={() => { setGenDestType("PHONE_CALL"); }}
                    className={`px-3 py-2.5 rounded-xl text-sm font-medium border transition-all flex items-center justify-center gap-1.5 ${genDestType === "PHONE_CALL" ? "bg-orange-500/10 border-orange-500/30 text-orange-400" : "bg-white/[0.03] border-white/[0.08] text-gray-500 hover:text-gray-300"}`}>
                    <Phone className="w-3.5 h-3.5" /> Phone Calls
                  </button>
                  <button type="button" onClick={() => { setGenDestType("MESSAGING"); if (genMsgApps.length === 0) setGenMsgApps(["MESSENGER"]); }}
                    className={`px-3 py-2.5 rounded-xl text-sm font-medium border transition-all flex items-center justify-center gap-1.5 ${genDestType === "MESSAGING" ? "bg-blue-500/10 border-blue-500/30 text-blue-400" : "bg-white/[0.03] border-white/[0.08] text-gray-500 hover:text-gray-300"}`}>
                    <MessagesSquare className="w-3.5 h-3.5" /> Messaging
                  </button>
                </div>

                {/* MESSAGING — multi-select checkboxes */}
                {genDestType === "MESSAGING" && (
                  <div className="mt-2 space-y-2">
                        ...
                        {[
                          ...
                          <label key={app.key} className="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" checked={genMsgApps.includes(app.key)}
                              onChange={(e) => {
                                if ((e.target as HTMLInputElement).checked) setGenMsgApps([...genMsgApps, app.key]);
                    ...
                    {genMsgApps.includes("WHATSAPP") && (
                      <input value={genWaNumber} onChange={(e) => setGenWaNumber(e.target.value)}
                        placeholder="+92 300 1234567"
                        className="w-full px-4 py-2.5 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white placeholder-gray-600 text-sm focus:outline-none focus:border-emerald-500/40 transition-all" />
                    )}
                ...
                {/* PHONE CALL */}
                {genDestType === "PHONE_CALL" && (
                  <div className="mt-2">
                    <input value={genCallPhone} onChange={(e) => setGenCallPhone(e.target.value)}
                      placeholder="+92 300 1234567"
                      className="w-full px-4 py-2.5 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white placeholder-gray-600 text-sm focus:outline-none focus:border-orange-500/40 transition-all" />
                    <p className="mt-1 text-[10px] text-gray-500">Users will see a &quot;Call Now&quot; button on the ad.</p>
                  </div>
                )}

                {/* WHATSAPP (legacy — kept for backward compat when old drafts exist) */}
                {genDestType === "WHATSAPP" && (
                  <input
                    value={genWaNumber}
                    onChange={(e) => setGenWaNumber(e.target.value)}
                    placeholder="+92 300 1234567"
                    className="w-full mt-2 px-4 py-2.5 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white placeholder-gray-600 text-sm focus:outline-none focus:border-emerald-500/40 transition-all"
                  />
                )}
                {genDestType === "INSTANT_FORM" && (
                  <div className="mt-2 space-y-2">
                    {genLeadFormId ? (
                      <div className="flex items-center justify-between px-3 py-2.5 rounded-xl bg-cyan-500/[0.08] border border-cyan-500/20">
                        <div className="flex items-center gap-2">
                          <Check className="w-3.5 h-3.5 text-cyan-400" />
                          <span className="text-xs text-cyan-300 truncate">{genLeadFormName || genLeadFormId}</span>
                        </div>
                        <button type="button" onClick={() => { setGenLeadFormId(""); setGenLeadFormName(""); setAiFormDraft(null); }}
                          className="text-[10px] text-gray-500 hover:text-gray-300">Change</button>
                      </div>
                    ) : aiFormDraft ? (
                      <div className="space-y-2">
                        <div className="flex items-start gap-2 bg-cyan-500/[0.05] border border-cyan-500/10 rounded-xl px-3 py-2">
                          <Sparkles className="w-3 h-3 text-cyan-400 mt-0.5 shrink-0" />
                          <p className="text-[10px] text-gray-400">{aiFormDraft.reasoning}</p>
                        </div>
                        <div className="px-3 py-2 rounded-xl bg-white/[0.02] border border-white/[0.06]">
                          <p className="text-xs text-white font-medium mb-1.5">{aiFormDraft.form_name}</p>
                          <div className="flex flex-wrap gap-1">
                            {aiFormDraft.questions.map((q: any, i: number) => (
                              <span key={i} className="px-2 py-0.5 rounded-lg bg-cyan-500/10 text-[10px] text-cyan-300 border border-cyan-500/15">
                                {q.label || q.type.replace(/_/g, " ")}
                              </span>
                            ))}
                          </div>
                        </div>
                        <button type="button" onClick={async () => {
                          if (!leadFormPageId) { setLeadFormError("No Facebook page found."); return; }
                          setLeadFormCreating(true); setLeadFormError(null);
                          try {
                            const res = await api.createLeadForm({ page_id: leadFormPageId, form_name: aiFormDraft.form_name, questions: aiFormDraft.questions, save_form: true });
                            const d = (res as any).data;
                            if (d?.form_id) { setGenLeadFormId(d.form_id); setGenLeadFormName(d.form_name); }
                            else setLeadFormError("No form ID returned.");
                          } catch (err: any) { setLeadFormError(err?.response?.data?.detail || "Failed to create form."); }
                          setLeadFormCreating(false);
                        }} disabled={leadFormCreating}
                          className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-xl text-xs font-semibold bg-gradient-to-r from-cyan-500 to-blue-500 text-white transition-all disabled:opacity-40">
                          {leadFormCreating ? <Loader2 className="w-3 h-3 animate-spin" /> : <Check className="w-3 h-3" />}
                          Approve & Create on Meta
                        </button>
                      </div>
                    ) : (
                      <div className="space-y-2">
                        {savedLeadForms.length > 0 && (
                          <div>
                            <p className="text-[10px] text-gray-500 mb-1">Select a saved form:</p>
                            {savedLeadForms.map((f) => (
                              <button key={f.meta_form_id} type="button"
                                onClick={() => { setGenLeadFormId(f.meta_form_id); setGenLeadFormName(f.form_name); }}
                                className="w-full text-left flex items-center gap-2 px-3 py-2 rounded-xl bg-white/[0.02] border border-white/[0.06] hover:bg-white/[0.04] transition-all mb-1">
                                <ClipboardList className="w-3 h-3 text-cyan-400 shrink-0" />
                                <span className="text-xs text-white truncate">{f.form_name}</span>
                                <span className="text-[10px] text-gray-600 ml-auto shrink-0">{f.questions?.length || 0} fields</span>
                              </button>
                            ))}
                          </div>
                        )}
                        {savedLeadForms.length === 0 && (
                          <p className="text-xs text-gray-500 text-center py-2">No saved forms yet. Create one from the Drafts page.</p>
                        )}
                      </div>
                    )}
                    {leadFormError && <p className="text-[10px] text-red-400">{leadFormError}</p>}
                  </div>
                )}
                <p className="text-[11px] text-gray-600 mt-1">
                  {genDestType === "MESSAGING" ? `Messaging — ${genMsgApps.join(", ").replace(/INSTAGRAM_DM/g, "IG Direct")}`
                    : genDestType === "PHONE_CALL" ? "Ads will show a Call Now button."
                    : genDestType === "WHATSAPP" ? "Ads will drive traffic to WhatsApp chat (COD mode)."
                    : genDestType === "INSTAGRAM_DM" ? "Ads will open an Instagram DM conversation."
                    : genDestType === "INSTANT_FORM" && genLeadFormId ? `Form attached: ${genLeadFormName}`
                    : genDestType === "INSTANT_FORM" ? "Collect leads in-app via Meta forms."
                    : "Ads will link to your product landing page."}
                </p>
              </div>
