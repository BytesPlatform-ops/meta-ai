...
/* DATE_PRESETS moved into DateRangePicker component */
      ...
      {!loading && !error && s && !s.no_data && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
            ...
            <StatCard
              label={rm.costLabel}
              value={rm.isCost ? (s.cost_per_result ? `$${s.cost_per_result.toFixed(2)}` : "--") : (s.roas != null ? `${s.roas.toFixed(2)}x` : "--")}
              icon={TrendingUp}
              color="bg-violet-500/10 text-violet-400"
            />
            <StatCard label={rm.label} value={String(s.results)} icon={Target} color="bg-amber-500/10 text-amber-400" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <GlassCard>
                <h3 className="text-sm font-semibold text-gray-300 mb-4">
                  {rm.isCost ? `${rm.costLabel} Trend` : "ROAS Trend"}
                </h3>
                <ResponsiveContainer width="100%" height={260}>
                  <LineChart data={data!.daily}>
                    ...
                    <Line type="monotone" dataKey={rm.isCost ? "cost_per_result" : "roas"} stroke="#8b5cf6" strokeWidth={2} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              </GlassCard>
                    ...
                    .map((row, i) => {
                      ...
                      return (
                        <div key={i} className="flex items-center gap-3 text-xs">
                          ...
                          <span className="w-12 text-right text-gray-500">{row.results} {rm.label.charAt(0)}</span>
                        </div>
                      );
                    })}
                </div>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-white/[0.06]">
                      ...
                      <th className="text-right text-gray-500 font-medium py-2 px-2">{rm.costLabel}</th>
                      <th className="text-right text-gray-500 font-medium py-2 px-2">CTR</th>
                      <th className="text-right text-gray-500 font-medium py-2 px-2">{rm.label}</th>
