...
export function DashboardStats() {
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const { data: overview } = await api.getDefaultOverview(since, until);
        setData(overview);
      } catch (err) {
  ...
  // Dynamic period label based on selected range
  ...
  const performanceCard = isLeadCampaign
    ? {
        label: `Cost per Lead (${periodLabel})`,
  ...
  const resultsLabel = isLeadCampaign ? `Leads (${periodLabel})` : data.results_30d > 0 ? `Results (${periodLabel})` : `Purchases (${periodLabel})`;
