...
async def trigger_audit(
    ...
    accounts = supabase.table("ad_accounts").select("id").eq("workspace_id", workspace_id).eq("is_active", True).limit(1).execute()
    ...
    try:
        audit = await run_audit(user_id, ad_account_id, workspace_id=workspace_id, status_filter=status_filter)
        return audit
