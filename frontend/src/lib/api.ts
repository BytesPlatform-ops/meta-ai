...
export const api = {
  ...
  /** AI Generation */
  generateDrafts: (count: number = 3, productId?: string, abTest?: boolean, userGuidance?: string, conversionEvent?: string) => {
  ...
  updateDraft: (id: string, data: { headline?: string; body_text?: string; image_url?: string; cta_type?: string; proposed_budget?: number; draft_type?: string; targeting?: Record<string, unknown>; pixel_id?: string; conversion_event?: string; thumbnail_url?: string }) =>
  ...
  listCampaigns: (adAccountId: string, statusFilter: string = "all", limit: number = 25) =>
    apiClient.get(`/api/v1/campaigns/${adAccountId}/list`, {
      params: { status_filter: statusFilter, limit },
    }),
  getCampaignInsights: (adAccountId: string, campaignId: string, datePreset: string = "last_7d") =>
    apiClient.get(`/api/v1/campaigns/${adAccountId}/insights/${campaignId}`, {
      params: { date_preset: datePreset },
    }),
  getCampaignDetail: (adAccountId: string, campaignId: string, datePreset: string = "last_7d") =>
    apiClient.get(`/api/v1/campaigns/${adAccountId}/detail/${campaignId}`, {
      params: { date_preset: datePreset },
    }),
  listAds: (adAccountId: string, campaignId: string, datePreset: string = "last_7d", statusFilter: string = "all") =>
    apiClient.get(`/api/v1/campaigns/${adAccountId}/ads/${campaignId}`, {
      params: { date_preset: datePreset, status_filter: statusFilter },
    }),
  ...
  getDefaultTimeSeries: (datePreset: string = "last_30d") =>
